...
EDSR 메인 모듈
"""
from options.EDSROptions import EDSROptions
...
# 옵션 저장 및 전역 변수 선언
opt = EDSROptions().getOptions()
evalopt = EDSROptions().getOptions()
evalopt.mode = "Eval"
evalopt.batch_size = 1
dataLoader = createDataset("EDSR", opt)
evalDataLoader = createDataset("EDSR", evalopt)
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + 1):
        ...
        with tqdm.tqdm(total=len(dataLoader), ncols=160) as t:
            ...
            for i, v in enumerate(dataLoader):
                ...
                # 상태바 업데이트
                t.set_postfix(loss='{:.6f}'.format(losses["EDSR"]))
